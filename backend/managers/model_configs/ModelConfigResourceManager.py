"""Model Config Resource Manager - model configuration CRUD operations."""
import json
...
class ModelConfigResourceManager(IResourceManager):
    ...
    def _list(self, req: RequestResourceModel) -> ResponseModel:
        try:
            ...
            items = self._db.model_configs.list_all(org_id=org_id)
            configs = [ModelConfigItem.from_item(i).to_api_dict() for i in items]
            status = req.data.get("status")
            if status:
                configs = [c for c in configs if c["status"] == status]
            return ResponseModel(success=True, data={"modelConfigs": configs})
    ...
    def _get(self, req: RequestResourceModel) -> ResponseModel:
        try:
            org_id = self._get_org_id(req.user_id)
            config_id = req.data.get("config_id")
            item = self._db.model_configs.get_by_key({"org_id": org_id, "id": config_id})
            if not item:
                return ResponseModel(success=False, error="Model config not found", status_code=404)
            return ResponseModel(success=True, data={"modelConfig": ModelConfigItem.from_item(item).to_api_dict()})
        except Exception as e:
            return ResponseModel(success=False, error=str(e), status_code=500)
    ...
    def _create(self, req: RequestResourceModel) -> ResponseModel:
        try:
            ...
            if model_config and not isinstance(model_config, str):
                model_config = json.dumps(model_config)
            config = ModelConfigItem(
                org_id=org_id,
                name=req.data.get("name", ""),
                model_type_id=req.data.get("modelTypeId"),
                model_config=model_config,
            )
            self._db.model_configs.create(config.to_item())
            return ResponseModel(success=True, data={"modelConfig": config.to_api_dict()}, status_code=201)
        except Exception as e:
            return ResponseModel(success=False, error=str(e), status_code=500)
    ...
    def _update(self, req: RequestResourceModel) -> ResponseModel:
        try:
            ...
            config_id = req.data.get("config_id")
            updates = {}
            for field, api_field in [("name", "name"), ("model_type_id", "modelTypeId"),
                                      ("status", "status")]:
                if api_field in req.data:
                    updates[field] = req.data[api_field]
            if "modelConfig" in req.data:
                val = req.data["modelConfig"]
                updates["model_config"] = json.dumps(val) if not isinstance(val, str) else val
            if not updates:
            ...
            for i, (k, v) in enumerate(updates.items()):
                ...
                values[f":v{i}"] = v
            kwargs = {
                "Key": {"org_id": org_id, "id": config_id},
                "UpdateExpression": "SET " + ", ".join(expr_parts),
                "ExpressionAttributeValues": values,
            }
            if names:
                kwargs["ExpressionAttributeNames"] = names
            self._db.model_configs.raw_update_item(**kwargs)
            return ResponseModel(success=True, message="Model config updated")
        except Exception as e:
            return ResponseModel(success=False, error=str(e), status_code=500)
    ...
    def _delete(self, req: RequestResourceModel) -> ResponseModel:
        try:
            org_id = self._get_org_id(req.user_id)
            config_id = req.data.get("config_id")
            self._db.model_configs.delete_by_key({"org_id": org_id, "id": config_id})
            return ResponseModel(success=True, message="Model config deleted")
        except Exception as e:
            return ResponseModel(success=False, error=str(e), status_code=500)
